...
namespace JY.BLL
{
	...
	{
		private readonly JY.DAL.InformationType dal=new JY.DAL.InformationType();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public JY.Model.InformationType GetModelByCache(long ID)
		{
			
			string CacheKey = "InformationTypeModel-" + ID;
			...
			return (JY.Model.InformationType)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<JY.Model.InformationType> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<JY.Model.InformationType> DataTableToList(DataTable dt)
		{
			List<JY.Model.InformationType> modelList = new List<JY.Model.InformationType>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				JY.Model.InformationType model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new JY.Model.InformationType();
