...
namespace JY.BLL
{
	...
	{
		private readonly JY.DAL.ReceiptAddress dal=new JY.DAL.ReceiptAddress();
		...
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public JY.Model.ReceiptAddress GetModelByCache(long ID)
		{
			
			string CacheKey = "ReceiptAddressModel-" + ID;
			...
			return (JY.Model.ReceiptAddress)objModel;
		}
		...
		/// 获得数据列表
		/// </summary>
		public List<JY.Model.ReceiptAddress> GetModelList(string strWhere)
		{
		...
		/// 获得数据列表
		/// </summary>
		public List<JY.Model.ReceiptAddress> DataTableToList(DataTable dt)
		{
			List<JY.Model.ReceiptAddress> modelList = new List<JY.Model.ReceiptAddress>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				JY.Model.ReceiptAddress model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = new JY.Model.ReceiptAddress();
					...
					if(dt.Rows[n]["LiveAdderss"]!=null && dt.Rows[n]["LiveAdderss"].ToString()!="")
					{
					model.LiveAdderss=dt.Rows[n]["LiveAdderss"].ToString();
					}
